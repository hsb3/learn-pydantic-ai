...
The 14 lessons are grouped into six phases. Each phase has its own payoff — you can stop after any phase and have something useful.
...
| **6 — Observability** | 14 | Trace every agent / tool / model / HTTP call as a nested span tree in Logfire. |

## How to study each lesson

Each lesson lives in its own directory (`lessons/NN_<slug>/`) with the README and the runnable file side by side. Lessons 01 and 13 are paired Jupyter notebooks (`.py` ↔ `.ipynb`) so you can poke them live; the rest ship as scripts. Lesson length is ~300 words; the tour in Lesson 01 is longer because it's reference. The pattern that works:
...
3. **Run** the example. From the repo root: `make intro-NN` (for Lessons 01 / 13 — notebooks — that prints a pointer; open in VS Code or run `make nb-exec`).
...
Resist the urge to read all 14 in a row. The lessons assume you've actually run the code from the previous one.
...
Start with [Lesson 01 — Agent API tour](../01_agent_api_tour/README.md).
