...
Each lesson lives in its own directory (`lessons/NN_<slug>/`) with the README and the runnable file side by side. Lesson 01's companion is a Jupyter-style notebook (`01_agent_api_tour.py` / `.ipynb`) so you can poke the API live; Lessons 02-12 ship as scripts. Lesson length is ~300 words; the tour in Lesson 01 is longer because it's reference. The pattern that works:
