...
**Code:** `examples/03_structured_output.py`
...
## Walk the code
- `examples/03_structured_output.py:17` — `CityLocation` is just a Pydantic `BaseModel`. `Field(description=...)` text is included in the schema sent to the model, so it actually shapes the answer.
- `examples/03_structured_output.py:24` — `Agent(FLASH, output_type=CityLocation)`. No tools, no special config — the type alone is the contract.
- `examples/03_structured_output.py:30` — `out: CityLocation = result.output`. The variable is typed; your editor knows `out.latitude` is a `float`.
