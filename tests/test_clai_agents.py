...
- examples/cli_agent.yaml         Google Gemini, WebSearch + light Thinking
- examples/clai_anthropic.yaml    Claude Sonnet 4.6, WebSearch + code_execution
...
def test_cli_agent_responds() -> None:
    r = _pai("examples/cli_agent.yaml", "What is 2 + 2? One word answer.")
...
def test_claude_agent_responds() -> None:
    r = _pai("examples/clai_anthropic.yaml", "What is 2 + 2? One word answer.")
    assert r.returncode == 0, r.stdout + r.stderr
    assert "4" in r.stdout or "four" in r.stdout.lower()
